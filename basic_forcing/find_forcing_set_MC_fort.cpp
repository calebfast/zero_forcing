...
int main(int argc, char* argv[]){
  ...
  /******File Reader*****************************************************************/
	...
	//Gurobi model********************************************************************************
    auto chrono_model_build_start = std::chrono::high_resolution_clock::now();
...
//*/
 ...
 // error = GRBsetintparam(GRBgetenv(zero_forcing), "RINS", 5);
  ...
  error = GRBupdatemodel(zero_forcing);
  auto chrono_model_build_end = std::chrono::high_resolution_clock::now();
  clock_t time_start = clock();
...
cout << "time finding forts was " << mydata.time_in_LP << endl;
  ...
  std::chrono::duration<double> chrono_model_build_time = chrono_model_build_end - chrono_model_build_start;
  ...
  fout << "Self-calculated time was: " << total_time << endl;
    ...
    fout << "Model building time was: " << chrono_model_build_time.count() << endl; 
    fout << endl;
	fout.close();
