...
the fort cover IP and MTZ connectivity constraints

Author: Caleb Fast
Date: 2017

Compilation: g++ -std=c++11 -o find_connected_forcing_set_MTZ find_connected_forcing_set_MTZ.cpp -L<path to gurobi libraries> -l<gurobi###> -lgurobi_c++
Example: g++ -std=c++11 -o find_connected_forcing_set_MTZ find_connected_forcing_set_MTZ.cpp -L/opt/gurobi651/linux64/lib -lgurobi65 -lgurobi_c++

Usage: ./find_connected_forcing_set_MTZ <path to .edg file>
...
/* Creates a GraphViz file that represent a branch decomposition tree.*/
void GraphVizBDecomp(forcing::node* bnodes, const int num_bnodes, string filename){

    double node_value, edge_value;
    int node_label;
    ofstream gv_file;

    //Begin by writing preamble
    gv_file.open((filename + ".gv").c_str());
    if(gv_file.is_open()){
    gv_file << "##This file encodes a branch decomposition tree " << filename << endl;
    gv_file << "##Command to generate layout is: neato -Tpng thisfile > thisfile.png" << endl;
    gv_file << "##Created by the cflib function GraphVizBDecomp()" << endl << endl;

    gv_file << "graph " << filename << " {" << endl;

    //Write the nodes
    for(int i=0; i < num_bnodes; i++){
      gv_file << i << " [label=\"" << i << "\"];" << endl;
    }

    //Write the edges
    for(int i=0; i < num_bnodes; i++){
      for(int j=0; j<bnodes[i].get_degree(); ++j){
	gv_file << i << "--" << bnodes[i].get_adj_list_member(j) << ";" << endl;
      }
    }

    gv_file << "overlap=false;" << endl;
    gv_file << "label=\"" << filename << "\";" << endl;
...
bool check_connectivity(std::set<int>& chosen_nodes, std::vector<int>& positive_edges, forcing::graph& our_graph){
...
int __stdcall cut_callback(GRBmodel *model, void *cbdata, int where, void *usrdata){
  ...
  if(where == GRB_CB_MIPSOL){
    ...
    for(std::set<std::set<int> >::iterator it = forts.begin(); it != forts.end(); ++it){
      //int v = mydata->our_graph.determine_facet_LP(mydata->env, *it);
  ...
  */
    ...
    is_connected = check_connectivity(chosen_nodes, positive_edges, mydata->our_graph);
...
*/
 ...
 // }*/
    ...
    std::chrono::duration<double> chrono_time = time_end - time_start;
    mydata->time_in_LP += chrono_time.count();
  }
...
int main(int argc, char* argv[]){
  ...
  //Loop over branch decomposition tree vertices to find forts.
  GraphVizBDecomp(&bd_tree.nodes[0], bnodes, "Branch_Decomposition");
  cout << "Finished branch decomposition" << endl;
  cout << "Branchwidth is " << branch_width << endl;

  for(int i=0; i<bd_tree.nodes.size();++i){
    const int degree = bd_tree.nodes[i].get_degree();
    cout << "Currently checking node " << i << endl;
    //Only check interior vertices
    if(degree > 1){
      //Look through the edges
      for(int j=0; j<degree; ++j){
        const int neighbor = bd_tree.nodes[i].get_adj_list_member(j);
        const int neighbor_degree = bd_tree.nodes[neighbor].get_degree();
        if(neighbor_degree > 1){
          bool is_greedy = false;
          for(int k=0; k<neighbor_degree; ++k){
            const int second_neighbor = bd_tree.nodes[neighbor].get_adj_list_member(k);
            if(bd_tree.nodes[second_neighbor].get_degree() == 1){
              is_greedy = true;
            }
          }
          //If no second neighbor through the edge is a leaf, then the edge gives a fort
          if(!is_greedy){
            cout << "node " << i << " is not greedy" << endl;
            std::queue<int> nodes_in_subtree;
            std::set<int> induced_nodes;
            std::set<int> new_fort;

            nodes_in_subtree.push(neighbor);
            std::set<int> forbidden_nodes;
            forbidden_nodes.insert(i);
            int counter = 0;
            while(!nodes_in_subtree.empty()){
              counter++;
              //cout << "in queue: counter is " << counter << endl;
              const int current_node = nodes_in_subtree.front();
              //cout << "current node is " << current_node << endl;
              const int current_node_degree = bd_tree.nodes[current_node].get_degree();
              for(int k=0; k<current_node_degree; ++k){
                const int second_neighbor = bd_tree.nodes[current_node].get_adj_list_member(k);
                if(forbidden_nodes.count(second_neighbor) != 1){
                  if(bd_tree.nodes[second_neighbor].get_degree() == 1){
                    induced_nodes.insert(second_neighbor);
                  }
                  else{
                    nodes_in_subtree.push(second_neighbor);
                  }
                }
              }
              forbidden_nodes.insert(current_node);
              nodes_in_subtree.pop();
            }

            //Now translate from leaves of decomposition to nodes of graph
            for(int i=0; i<end1.size(); ++i){
              const int nu_i = nu[i];
              if(induced_nodes.count(nu_i) == 1){
                new_fort.insert(end1[i]);
                new_fort.insert(end2[i]);
              }
            }
            cout << "fort.size() is " << new_fort.size() << endl;
            //our_graph.make_fort_minimal(new_fort);
            cout << "reduced fort.size() is " << new_fort.size() << endl;
            std::pair<std::set<std::set<int> >::iterator, bool> return_pair = forts.insert(new_fort);
            if(!return_pair.second){
              cout << "Failed to insert new fort" << endl;
              cout << "new_fort.size is " << new_fort.size() << endl;
            }
          }
        }
      }
  ...
  cout << "Finished finding BD forts" << endl;
  cout << "number of branch decomposition forts was " << forts.size() << endl;

  //*/

  forcing::graph MTZ_graph;
  MTZ_graph.nodes = our_graph.nodes;
  for(int i=0; i<our_graph.edges.get_size(); ++i){
    MTZ_graph.edges.add_edge(our_graph.edges.get_end1(i), our_graph.edges.get_end2(i));
  }
  MTZ_graph.nodes.push_back(blank_node);
  MTZ_graph.nodes.push_back(blank_node);
  MTZ_graph.nodes[num_nodes].add_to_adj_list(num_nodes+1);
  MTZ_graph.edges.add_edge(num_nodes, num_nodes+1);
  for(int i=0; i<num_nodes; ++i){
    MTZ_graph.nodes[num_nodes].add_to_adj_list(i);
    MTZ_graph.edges.add_edge(num_nodes, i);
    MTZ_graph.nodes[num_nodes+1].add_to_adj_list(i);
    MTZ_graph.edges.add_edge(num_nodes+1, i);
  }
  cout << "num_edges is " << our_graph.edges.get_size() << endl;
  cout << "num MTZ nodes is " << MTZ_graph.nodes.size() << endl;
  cout << "num MTZ edges is " << MTZ_graph.edges.get_size() << endl;
	//Gurobi model********************************************************************************
    ...
    const int num_variables = num_nodes + MTZ_graph.edges.get_size() + MTZ_graph.nodes.size();
    const int MTZ_u_start = num_nodes;
    const int MTZ_y_start = MTZ_u_start + MTZ_graph.nodes.size();
  ...
  cout << "mydata.our_graph edge_list.size() is " << mydata.our_graph.edges.get_size() << endl;
    ...
    for(int i=num_nodes; i<MTZ_y_start; ++ i){
      types[i] = GRB_INTEGER;
      //types[i] = GRB_CONTINUOUS;
      //Objective sums all nodes at first time stage
      objective_coeffs[i] = 0;
      lower_bounds[i] = 1;
      if(i == MTZ_u_start + num_nodes){
        lower_bounds[i] = 0;
      }
      upper_bounds[i] = num_nodes+1;
      branching_priority[i] = 4;
    }
    for(int i=MTZ_y_start; i<num_variables; ++ i){
      types[i] = GRB_BINARY;
      //types[i] = GRB_CONTINUOUS;
      //Objective sums all nodes at first time stage
      objective_coeffs[i] = 0;
 ...
 /*Constraint 1: Add fort constraints that do not contain subset variables*
  ...
  //Constraint 2a
  {
    ...
    std::vector<double> cval;
    for(int i=0; i<num_nodes; ++i){
      cind.push_back(MTZ_y_start + MTZ_graph.edges.get_edge_number(num_nodes+1, i));
      cval.push_back(1.0);
    }
    error = GRBaddconstr(zero_forcing, cind.size(), &cind[0], &cval[0], GRB_EQUAL, 1.0, NULL);
    if(error){
      std::cerr << "ERROR: " << error << " at 2a constraint" << endl;
      return 1;
    }
  }
  ...
  for(int i=0; i<num_nodes; ++i){
    ...
    for(int j=0; j<node_degree; ++j){
      cind.push_back(MTZ_y_start + MTZ_graph.edges.get_edge_number(our_graph.nodes[i].get_adj_list_member(j),i));
      cval.push_back(1.0);
    }
    cind.push_back(MTZ_y_start + MTZ_graph.edges.get_edge_number(num_nodes,i));
    cval.push_back(1.0);
    cind.push_back(MTZ_y_start + MTZ_graph.edges.get_edge_number(num_nodes+1,i));
    cval.push_back(1.0);
    error = GRBaddconstr(zero_forcing, cind.size(), &cind[0], &cval[0], GRB_EQUAL, 1.0, NULL);
  ...
  for(int i=0; i<our_graph.edges.get_size(); ++i){
    std::vector<int> cind;
    std::vector<double> cval;
    cind.push_back(MTZ_y_start + MTZ_graph.edges.get_edge_number(num_nodes,our_graph.edges.get_end1(i)));
    cval.push_back(1.0);
    cind.push_back(MTZ_y_start + MTZ_graph.edges.get_edge_number(our_graph.edges.get_end1(i), our_graph.edges.get_end2(i)));
    cval.push_back(1.0);
    error = GRBaddconstr(zero_forcing, cind.size(), &cind[0], &cval[0], GRB_LESS_EQUAL, 1.0, NULL);
    if(error){
      std::cerr << "ERROR: " << error << " at 2c constraint" << endl;
      return 1;
    }
  }
  ...
  for(int i=0; i<our_graph.edges.get_size(); ++i){
    ...
    std::vector<double> cval;
    cind.push_back(MTZ_y_start + MTZ_graph.edges.get_edge_number(our_graph.edges.get_end1(i), our_graph.edges.get_end2(i)));
    cval.push_back(num_nodes+1);
    cind.push_back(MTZ_y_start + MTZ_graph.edges.get_edge_number(our_graph.edges.get_end2(i),our_graph.edges.get_end1(i)));
    cval.push_back(num_nodes - 1);
    cind.push_back(MTZ_u_start + our_graph.edges.get_end1(i));
    cval.push_back(1.0);
    cind.push_back(MTZ_u_start + our_graph.edges.get_end2(i));
    cval.push_back(-1.0);
    error = GRBaddconstr(zero_forcing, cind.size(), &cind[0], &cval[0], GRB_LESS_EQUAL, num_nodes, NULL);
    if(error){
      std::cerr << "ERROR: " << error << " at 2d constraint" << endl;
      return 1;
    }
  ...
  //Constraint 2e
  for(int i=our_graph.edges.get_size(); i<MTZ_graph.edges.get_size(); ++i){
    std::vector<int> cind;
    std::vector<double> cval;
    cind.push_back(MTZ_y_start + i);
    cval.push_back(num_nodes+1);
    cind.push_back(MTZ_u_start + MTZ_graph.edges.get_end1(i));
    cval.push_back(1.0);
    cind.push_back(MTZ_u_start + MTZ_graph.edges.get_end2(i));
    cval.push_back(-1.0);
    error = GRBaddconstr(zero_forcing, cind.size(), &cind[0], &cval[0], GRB_LESS_EQUAL, num_nodes, NULL);
    if(error){
      std::cerr << "ERROR: " << error << " at 2a constraint" << endl;
  ...
  //Constraint 2f
  {
    std::vector<int> cind;
    std::vector<double> cval;
    cind.push_back(MTZ_y_start + MTZ_graph.edges.get_edge_number(num_nodes, num_nodes+1));
    cval.push_back(1.0);
    error = GRBaddconstr(zero_forcing, cind.size(), &cind[0], &cval[0], GRB_EQUAL, 1.0, NULL);
    if(error){
      std::cerr << "ERROR: " << error << " at 2f constraint" << endl;
      return 1;
    }
  }
  ...
  //Constraint 2g
  {
    std::vector<int> cind;
    std::vector<double> cval;
    cind.push_back(MTZ_u_start + num_nodes);
    cval.push_back(1.0);
    error = GRBaddconstr(zero_forcing, cind.size(), &cind[0], &cval[0], GRB_EQUAL, 0, NULL);
    if(error){
      std::cerr << "ERROR: " << error << " at 2g constraint" << endl;
      return 1;
    }
  }//*/
  ...
  for(int i=0; i<num_nodes; ++i){
    ...
    error = GRBaddconstr(zero_forcing, cind.size(), &cind[0], &cval[0], GRB_EQUAL, 1.0, NULL);
    if(error){
      std::cerr << "ERROR: " << error << " at 2i constraint" << endl;
      return 1;
    }
  }
  ...
  //error = GRBsetintparam(GRBgetenv(zero_forcing), "GomoryPasses", 2);
